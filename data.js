window.PORTFOLIO_PROJECTS = [
  {
    ...
    "period": "2025.10 — 현재",
    "company": "그루핀",
    "role": "차장 · Flutter 앱 전체 개발·아키텍처 설계·대규모 리팩터링",
    ...
    "context": "EVM·Cosmos 멀티체인 지갑의 Android·iOS 앱 전체 개발과 아키텍처 설계, 대규모 리팩터링을 담당했습니다. 토큰·NFT·트랜잭션·인증·KYC·리워드의 연결 구조를 다루며, AI를 활용한 개발과 회귀·실기기 검증으로 구조 개선과 서비스 안정화를 함께 수행했습니다.",
    "decisions": [
      [
        "아키텍처 설계",
        "Riverpod·Freezed·Repository 기반 MVVM 아키텍처를 설계하고 자산·인증·트랜잭션 상태를 관리했습니다. 화면, 상태 관리와 데이터 접근의 역할을 나누어 기능 확장과 유지보수에 대응했습니다."
      ],
      [
        "대규모 리팩터링",
        "대형 인증 ViewModel과 중복 블록체인 로직을 분리하고, 여러 기능에 걸친 인증·자산·트랜잭션 처리의 역할을 정리했습니다. 구조 변경이 기존 지갑 동작에 미치는 영향을 회귀 테스트와 실기기 검증으로 점검했습니다."
      ],
      [
        "연결과 복구",
      ...
      [
        "출시 이후의 운영",
        "Google·Apple·Twitter 로그인, Firebase/ML Kit KYC, 6개 언어 국제화와 Shorebird OTA 환경을 구현했습니다. Flame 기반 Garu Miner와 출석·포인트·상점 기능도 개발했습니다. 게임·오디오·카메라·WebView의 생명주기와 리소스 해제도 점검했습니다."
      ],
      [
        "AI를 활용하는 방식",
        "Gemini·Claude·Codex를 활용해 변경 범위와 검증 기준을 구체화하고, 구현 결과를 코드 검토와 회귀 테스트, 실제 기기 동작으로 확인했습니다."
      ],
      [
        "테스트에서 실제 사용까지",
        "Flutter integration_test로 저장소·PIN 호환성을 점검하고 Android ADB E2E·iOS XCTest/XCUITest로 인증·외부 링크·권한 거부 후 복구를 검증했습니다. 외부 테스트 dApp의 EVM personal_sign 취소·PIN 인증·서명 응답을 확인하고, 반환 서명에서 복원한 주소가 연결 계정과 일치하는지 검증했습니다. 자동화한 항목과 직접 확인할 항목을 구분하고 실제 화면과 기대 결과를 대조했습니다."
      ],
      [
        "SDK 재초기화 후 대기 상태 복구",
        "WalletConnect SDK 재초기화가 로딩 상태를 해제하면서 연결 제안 타임아웃까지 누락되는 문제를 수정했습니다. 페어링 대기 상태를 복구하고 외부 링크의 오류 전달·동일 링크 재시도·중복 요청 방어를 보완했습니다. 관련 26개 파일의 204개 테스트 통과를 확인하고 Android 실기기와 iOS XCUITest에서 제안 미도착 오류와 재시도 동작을 검증했습니다."
      ],
      [
        "저장소 이관의 중단과 복구",
        "Android 보안 저장소의 네이티브 구현을 보완해 새 값의 저장·복호화 확인 후 구형 값을 삭제하도록 했습니다. 부분 이관 재시도와 갱신 값 보존을 포함해 합성 데이터 기반 실물 15개 시나리오, 관련 Dart 테스트 63개를 통과했습니다."
      ],
      [
        "기존 빌드 최적화와 릴리스 검증",
        "2026.08 집계의 R8 최적화에서는 resources.txt 78.16%, mapping.txt 0.91%, APK 114,872 B 감소를 확인했습니다. 당시 릴리스 업데이트·콜드 스타트 검증 범위에서 Crash·ANR·주요 플러그인 치명 오류 0건을 확인하고, Shorebird Code Push로 Dart 코드·번역 변경을 배포하는 OTA 환경을 구성했습니다. 후속 빌드 수치와 측정 대상·시점이 다릅니다."
      ],
      [
        "빌드 크기와 호환성",
        "R8 규칙·미사용 OCR 의존성·리소스를 정비하고 WebView 브리지와 네이티브 API를 릴리스에서 검증했습니다. 기존 61번 빌드 대비 DEX 27.71%, Note9 다운로드 예상 크기 16.65% 감소를 확인했습니다. Flutter 엔진 차이를 포함한 비교입니다."
      ],
      [
        "표시 크기에 맞춘 이미지 디코딩",
        "공통 이미지 위젯에 표시 크기·기기 픽셀 비율 기반 디코딩 제한을 적용했습니다. Note9에서 2048×1024 합성 PNG를 40×40 논리 픽셀로 표시한 비교에서 디코딩·캐시 메모리가 8 MiB에서 약 21 KiB로 줄었습니다. 단일 이미지 측정값입니다."
      ]
    ],
    "outcome": "아키텍처 설계·대규모 리팩터링을 회귀 검증과 연결하고, 네이티브 저장소 복구·빌드 및 이미지 최적화까지 개선 범위를 확장했습니다. 2026.08 집계에서는 기존 1개에서 전체 175개 테스트 파일로 확대했으며, 이 중 171개를 본인 커밋으로 추가했습니다. 개인 비병합 커밋 150건 중 66건은 결함 수정·QA·안정화 작업입니다. 2026.09 커밋 기준 단위·위젯 테스트 241개 파일과 통합 테스트 9개 파일을 유지하고 있습니다. 최근 저장소 복구·빌드·이미지 개선은 릴리스 빌드 검증 완료, 스토어 배포 전입니다."
  },
  {
    "id": "mindhill",
    "name": "Mindhill",
